...
from monai.data import to_affine_nd, affine_to_spacing
...
class AdjustGuidanceSpacing(MapTransform):
    def __init__(self, keys: KeysCollection, image_meta_dict_key = "image_meta_dict", allow_missing_keys: bool = False):
        super().__init__(keys, allow_missing_keys)
        self.image_meta_dict_key = image_meta_dict_key

    def __call__(self, data):
        d: Dict = dict(data)
        meta_data = d[self.image_meta_dict_key]
        affine_ = to_affine_nd(r=3, affine=convert_data_type(meta_data["original_affine"], np.ndarray)[0])
        old_pixdim = affine_to_spacing(affine_, r=3)
        old_size = d[self.shape_key][-3:]
        for key, channel in self.key_iterator(d, range(len(self.keys))):
            for i, point in enumerate(d[key]):
                d[key][i] = tuple(int(round(point[k] * old_pixdim[k] / self.pixdim[k] / old_size[k] * self.size[k])) for k in range(len(point)))
        return d


class EncodeBoundaryClickSignald(MapTransform):
    def __init__(self, keys : KeysCollection, image_key : str, spatial_size = (96,96,96), image_meta_dict_key = "image_meta_dict", allow_missing_keys : bool = False):
        super().__init__(keys, allow_missing_keys)
    ...
    def __call__(self, data):
        ...
        meta_dict = d[self.image_meta_dict_key]
        original_affine = to_affine_nd(r=3, affine=convert_data_type(meta_dict["original_affine"], np.ndarray)[0])
        original_pixdim = affine_to_spacing(original_affine, r=3)
        curr_pixdim = meta_dict['pixdim'][1:4].tolist()
        original_size = meta_dict["spatial_shape"].tolist()
        num_guidance_channels = len(self.keys)
        guidance_tensor = torch.zeros(num_guidance_channels, *d[self.image_key].shape[-3:], dtype=torch.float)
        channel = 0
        for key in self.key_iterator(data):
            points = d[key]
            for i, point in enumerate(d[key]):
                points[i] = tuple(int(round(point[k] * original_pixdim[k] / curr_pixdim[k] / original_size[k] * self.spatial_size[k])) for k in [0,1,2])
            print(points)
            if len(points) > 0:
                ...
                if (guidance_tensor[channel] != torch.inf).all():
                    guidance_tensor[channel] = 1.0 / (guidance_tensor[channel] + 1.1)
