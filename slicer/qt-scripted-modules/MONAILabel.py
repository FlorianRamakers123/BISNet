...
class MONAILabelWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    def __init__(self, parent=None):
        ...
        self.bisnetPositivePointListNode = None                     
        self.bisnetPositivePointListNodeObservers = []
    ...
    def setup(self):
        ...
        self.ui.bisnetPositiveControlPointPlacementWidget.setMRMLScene(slicer.mrmlScene)
        self.ui.bisnetPositiveControlPointPlacementWidget.placeButton().toolTip = "Select boundary points"
        self.ui.bisnetPositiveControlPointPlacementWidget.buttonsVisible = False
        self.ui.bisnetPositiveControlPointPlacementWidget.placeButton().show()
        self.ui.bisnetPositiveControlPointPlacementWidget.deleteButton().show()
        ...
        self.ui.bisnetUpdateButton.setIcon(self.icon("segment.png"))
        ...
        self.ui.bisnetUpdateButton.connect("clicked(bool)", self.onUpdateBISNet)        # TODO: Create this method
        self.ui.dgUpdateCheckBox.setStyleSheet("padding-left: 10px;")
        self.ui.bisnetUpdateCheckBox.setStyleSheet("padding-left: 10px;")
    ...
    def onSceneStartClose(self, caller, event):
        ...
        self.resetPointList(                                            
            self.ui.bisnetPositiveControlPointPlacementWidget,
            self.bisnetPositivePointListNode,
            self.bisnetPositivePointListNodeObservers,
        )
        self.bisnetPositivePointListNode = None
    ...
    def updateGUIFromParameterNode(self, caller=None, event=None):
        ...
        if self._segmentNode:
            ...
            logging.info("LABELS:" +  str(self.info.get("labels", [])))
        ...
        if self._segmentNode:
            ...
            if not self.bisnetPositivePointListNode:
                self.bisnetPositivePointListNode, self.bisnetPositivePointListNodeObservers = self.createPointListNode(
                    "B", self.onBISNetPointListNodeModified, [0.5, 1, 0.5]      
                )
                self.ui.bisnetPositiveControlPointPlacementWidget.setCurrentNode(self.bisnetPositivePointListNode)
                self.ui.bisnetPositiveControlPointPlacementWidget.setPlaceModeEnabled(False)

            self.ui.scribblesCollapsibleButton.setEnabled(self.ui.scribblesMethodSelector.count)
            self.ui.scribblesCollapsibleButton.collapsed = False

        logging.info("Enabling widgets")
        self.ui.bisnetPositiveControlPointPlacementWidget.setEnabled(True)
        ...
        self.ui.bisnetUpdateCheckBox.setEnabled(bool(self._segmentNode))
        self.ui.dgUpdateButton.setEnabled(self.ui.deepgrowModelSelector.currentText and self._segmentNode)
        self.ui.bisnetUpdateButton.setEnabled(bool(self._segmentNode))
    ...
    def onBISNetPointListNodeModified(self, observer, eventid):          
        logging.debug("BISNet Point Event!!")
        ...
        if not self.ui.bisnetUpdateCheckBox.checked:
            self.onClickBISNet(current_point, skip_infer=True)
            return

        self.onClickBISNet(current_point)

        self.ignorePointListNodeAddEvent = True
        self.onEditControlPoints(self.bisnetPositivePointListNode, "MONAILabel.ForegroundPoints")
        self.ignorePointListNodeAddEvent = False
    ...
    def onUpdateBISNet(self):
        self.onClickBISNet(None)

    def onClickBISNet(self, current_point, skip_infer=False):         
        model = "bisnet"
        if model not in self.models:
            slicer.util.warningDisplay(f"Please load a BISNet model {list(self.models.keys())}")
            return

        _, segment = self.currentSegment()
        if not segment:
            slicer.util.warningDisplay("Please add the required label to run BISNet")
            return

        foreground_all = self.getControlPointsXYZ(self.bisnetPositivePointListNode, "foreground")
        ...
        start = time.time()
        label = segment.GetName()
        operationDescription = f"Run BISNet for segment: {label}; model: {model}"
        logging.debug(operationDescription)

        if not current_point:
            current_point = foreground_all[-1] if foreground_all else None
        ...
        try:
            ...
            params["label"] = label
            params.update(self.getParamsFromConfig("infer", model))
            print(f"Request Params for BISNet: {params}")

            image_file = self.current_sample["id"]
            result_file, params = self.logic.infer(model, image_file, params, session_id=self.getSessionId())
            print(f"Result Params for BISNet: {params}")
            if labels is None:
                labels = (
                    params.get("label_names")
                    if params and params.get("label_names")
            ...
            freeze = None
            self.updateSegmentationMask(result_file, labels, None, freeze=freeze)
        except:
            logging.exception("Unknown Exception")
            slicer.util.errorDisplay(operationDescription + " - unexpected error.", detailedText=traceback.format_exc())
        finally:
        ...
        self.updateGUIFromParameterNode()
        logging.info(f"Time consumed by BISNet: {time.time() - start:3.1f}")
    ...
    def createPointListNode(self, name, onMarkupNodeModified, color):
        ...
        displayNode.SetGlyphScale(0.75)
